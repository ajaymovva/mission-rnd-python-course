...
def get_green_length(segments):
    if(len(segments)>0):
        ...
        while(i<len(segments)):
            if(segments[i][0]<end and segments[i][1]>end):
                length=length+(segments[i][1]-end)
                end=segments[i][1]
            elif(segments[i][0]>=end):
                length=length+segments[i][1]-segments[i][0]
                end=segments[i][1]
            i+=1
        return length
...
def test_get_green_length():
    ...
    assert 24 == get_green_length([(1,5),(3,4),(10,20),(20,30)])
    assert 999 == get_green_length([(1, 5),(2,3),(20, 1000), (1,30)])